...
void Camara::SetDirectionVector(float x, float y, float z)
{
    m_lx = x;
    m_ly = y;
    m_lz = z;
...
void Camara::GetYaw(float &angle)
{
	angle = m_yaw;
}
...
void Camara::GetPitch(float &angle)
{
    angle = m_pitch;
}

void Camara::SetCamera(float PosX, float PosY, float PosZ, float DirX, float DirY, float DirZ, float YawAngle, float PitchAngle)
{
	Camara::SetPos(PosX, PosY, PosZ);
	Camara::SetDirectionVector(DirX, DirY, DirZ);
	Camara::SetYaw(YawAngle);
	Camara::SetPitch(PitchAngle);
}